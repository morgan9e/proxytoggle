...
function settingsChanged(settings) {
  if ("proxySettings" in settings) proxies[1] = settings.proxySettings.newValue;
  if ("skipLocal" in settings) skipLocal = settings.skipLocal.newValue;
  if ("blacklist" in settings) blacklist = settings.blacklist.newValue;
  if ("whitelist" in settings) whitelist = settings.whitelist.newValue;
  if ("useWhitelist" in settings) useWhitelist = settings.useWhitelist.newValue;
  if ("useBlacklist" in settings) useBlacklist = settings.useBlacklist.newValue;
  if (useWhitelist) whitelistRegexes = compileRegexes(whitelist);
  if (useBlacklist) blacklistRegexes = compileRegexes(blacklist);
}
...
function compileRegexes(from) {
  return from.map(function (pattern) {
...
function provideCredentialsSync(requestDetails) {
  ...
  if (pendingRequests.indexOf(requestDetails.requestId) != -1) {
    ...
    console.log(
      "Bad proxy credentials for request: " + requestDetails.requestId,
    );
    return { cancel: true };
  }
  var credentials = {
  ...
  if (DEBUG) {
    console.log(
      `Providing proxy credentials for request: ${requestDetails.requestId} username: ${credentials.username}`,
    );
  }
...
function isLocalIPv4(host) {
  ...
  if (octets[1] > 255 || octets[2] > 255 || octets[3] > 255 || octets[4] > 255)
    return false;
  if (octets[1] == 10 || octets[1] == 127) return true;
...
function handleProxyRequest(requestInfo) {
  ...
  if (useWhitelist) {
    proxyNum = 0; // Default to no-proxy
    if (whitelistRegexes && whitelistRegexes.length > 0) {
      for (var i = 0; i < whitelistRegexes.length; i++) {
        var regex = whitelistRegexes[i];
        if (regex.test(url)) {
          if (DEBUG) console.log(`Whitelist: ${url} match ${whitelist[i]}`);
          proxyNum = 1;
          break;
        }
      }
  ...
  if (useBlacklist) {
    if (blacklistRegexes && blacklistRegexes.length > 0) {
      for (var i = 0; i < blacklistRegexes.length; i++) {
        ...
        if (regex.test(url)) {
          if (DEBUG) console.log(`Blacklist: ${url} match ${blacklist[i]}`);
          proxyNum = 0;
          break;
        }
      }
  ...
  if (skipLocal) {
    if (isLocal(host)) {
      if (DEBUG) console.log(`Local host detected: ${host}`);
      proxyNum = 0;
    }
  }
...
browser.storage.local.get(
  ...
  (items) => {
    ...
    whitelist = items.whitelist;
    useWhitelist = items.useWhitelist;
